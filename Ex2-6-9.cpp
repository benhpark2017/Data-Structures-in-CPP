/**
 * Solutions to Chapter 2, Section 6, Exercises 9 of Horowitz's
...
// Solution to Chapter 2, Section 6, Exercise 9
// Introducing the function definition for the new failure function. 
void String::fail() {
    if (f != nullptr) {
        delete[] f;
    }
    ...
    while (j < len - 1) {
        if (k == -1 || str[j] == str[k]) {
            j++;
            k++;
            if (str[j] != str[k]) f[j] = k;
            else f[j] = f[k];  // Optimization to skip redundant comparisons
        } else {
            k = f[k];
        }
    }
